...
import { resolveRegistryIcon, sanitizeIconHref } from '../common/components/RegistryIcon';
...
describe('cross-registry references use the qualified reference form', () => {
  it('names skills the same way in agent pins, plugin members, and URIs', () => {
    // A bare name here would match nothing and fail SILENTLY: the backlink sections would
    // render "nothing depends on this" for a skill that half the registry depends on.
    for (const pluginVersion of PLUGIN_VERSIONS) {
      for (const member of pluginVersion.members) {
        if (member.member_type === 'skill') {
          expect(member.name.startsWith('@')).toBe(true);
        }
      }
    }
  });

  it('resolves plugin members back to skills that actually exist', () => {
    const known = new Set(SKILLS.map((skill) => getSkillQualifiedName(skill.organization, skill.name)));
    const referenced = new Set(
      PLUGIN_VERSIONS.flatMap((pluginVersion) =>
        pluginVersion.members.filter((member) => member.member_type === 'skill').map((member) => member.name),
      ),
    );
    // The plugin seeds deliberately reference a wider catalog than the skills seeds carry,
    // so this asserts the overlap resolves rather than demanding every reference does.
    const resolvable = [...referenced].filter((name) => known.has(name));
    expect(resolvable.length).toBeGreaterThan(0);
  });
});

describe('icons identify a skill, not one of its registrations', () => {
  /*
    RFC-0008 PR #45 puts `icons` on the PARENT entity, which is what Bill asked for on the
    2026-09-01 call when the question was whether an icon belonged to the skill or to a
    version. A per-version icon would let one skill change appearance as the rail moves.
  */
  it('carries icons on the skill and never on a version', () => {
    const withIcon = SKILLS.filter((skill) => skill.icons?.length);
    expect(withIcon.length).toBeGreaterThan(0);
    for (const version of SKILL_VERSIONS) {
      expect(version).not.toHaveProperty('icons');
    }
  });

  // Some entries deliberately have none, so the default glyph is exercised too.
  it('leaves some skills without one', () => {
    expect(SKILLS.some((skill) => !skill.icons?.length)).toBe(true);
  });

  it('resolves the theme variant, falling back to an untagged icon', () => {
    const icons = [{ src: 'https://example.com/light.svg' }, { src: 'https://example.com/dark.svg', theme: 'dark' }];
    expect(resolveRegistryIcon(icons, true)?.src).toBe('https://example.com/dark.svg');
    expect(resolveRegistryIcon(icons, false)?.src).toBe('https://example.com/light.svg');
    expect(resolveRegistryIcon([{ src: 'https://example.com/only.svg' }], true)?.src).toBe(
      'https://example.com/only.svg',
    );
  });

  /*
    An icon is a publisher-supplied string rendered into an `img src`, so it is an
    injection surface. Anything that is not an ordinary web URL is dropped and the default
    glyph shows instead.
  */
  it('accepts only http and https URLs', () => {
    expect(sanitizeIconHref('https://example.com/icon.svg')).toBe('https://example.com/icon.svg');
    // eslint-disable-next-line no-script-url -- the rejected scheme IS the assertion
    expect(sanitizeIconHref('javascript:alert(1)')).toBeUndefined();
    expect(sanitizeIconHref('data:image/svg+xml;base64,PHN2Zz48L3N2Zz4=')).toBeUndefined();
    expect(sanitizeIconHref('not a url')).toBeUndefined();
    expect(sanitizeIconHref(undefined)).toBeUndefined();
  });
});
