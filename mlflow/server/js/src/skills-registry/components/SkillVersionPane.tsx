...
import { AgentsUsingSkillSection } from '../../agent-registry/components/AgentsUsingSkillSection';
import { PluginsContainingSkillSection } from '../../agent-plugins/components/PluginsContainingSkillSection';
import {
  SkillDigestCell,
  SkillSourceTypeTag,
  SkillStatusTag,
...
import { getGitBrowseUrl, getSkillAliasUri, getSkillQualifiedName, getSkillVersionUri } from '../constants';
 ...
 * tabs beneath it.
 *
 * On the composed branch the strip holds Files, "Used by agents" and "Packaged in plugins".
 * Traces is not here: Daniel Warner's 2026-09-07 pass moved it to the page-level
 * Preview/Traces switch above the rail, the prompt details arrangement. On the skills-only
 * branch that left Files alone, so there the strip went and Files became a grid row; here
 * the two reverse lookups keep the strip worth having (2026-09-11 ruling).
 *
 * Both reverse lookups are scoped to the SELECTED version: "which agents pin v3" is a
 * different answer from "which agents pin this skill at all", and the rail beside the pane
 * is what makes that difference visible.
 */
export enum SkillDetailTab {
  FILES = 'files',
  AGENTS = 'agents',
  PLUGINS = 'plugins',
}

const isSkillDetailTab = (value: string | null): value is SkillDetailTab =>
  value !== null && (Object.values(SkillDetailTab) as string[]).includes(value);
...
export const SkillVersionPane = ({
  ...
  // Files is the default and stays out of the URL, the same convention the agent and agent
  // plugin pages use for their own `tab` parameter.
  const [searchParams, setSearchParams] = useSearchParams();
  const tabParam = searchParams.get('tab');
  const activeTab = isSkillDetailTab(tabParam) ? tabParam : SkillDetailTab.FILES;
  const handleTabChange = (value: string) => {
    if (!isSkillDetailTab(value)) {
      return;
    }
    const next = new URLSearchParams(searchParams);
    if (value === SkillDetailTab.FILES) {
      next.delete('tab');
    } else {
      next.set('tab', value);
    }
  ...
  return (
    ...
    >
      ...
      {/*
        Tabs under the metadata grid, the MCP registry arrangement: everything that describes
        the version renders first and stays put, and the tabs sit inside the pane where their
        scope is obvious. Files is a tab here rather than the labelled grid row it is on the
        skills-only branch, because this branch has two more things to choose between.
      */}
      <Tabs.Root
        componentId="mlflow.skills-registry.version-pane.tabs"
        valueHasNoPii
        value={activeTab}
      ...
      >
        <Tabs.List>
          <Tabs.Trigger value={SkillDetailTab.FILES}>
            <FormattedMessage defaultMessage="Files" description="Skills registry > skill detail > files tab" />
          </Tabs.Trigger>
          <Tabs.Trigger value={SkillDetailTab.AGENTS}>
            <FormattedMessage
              defaultMessage="Used by agents"
              description="Skills registry > skill detail > agents tab"
            />
          </Tabs.Trigger>
          <Tabs.Trigger value={SkillDetailTab.PLUGINS}>
            <FormattedMessage
              defaultMessage="Packaged in plugins"
              description="Skills registry > skill detail > agent plugins tab"
            />
          </Tabs.Trigger>
        </Tabs.List>

        <Tabs.Content value={SkillDetailTab.FILES}>
          <SkillFilesTab skillVersion={skillVersion} totalVersions={totalVersions} />
        </Tabs.Content>

        <Tabs.Content value={SkillDetailTab.AGENTS}>
          <AgentsUsingSkillSection
            skillName={getSkillQualifiedName(skill.organization, skill.name)}
            skillVersion={skillVersion.version}
          />
        </Tabs.Content>

        <Tabs.Content value={SkillDetailTab.PLUGINS}>
          <PluginsContainingSkillSection
            qualifiedSkillName={getSkillQualifiedName(skill.organization, skill.name)}
            skillVersion={skillVersion.version}
          />
        </Tabs.Content>
      </Tabs.Root>
