...
describe('SkillListFilters', () => {
  it('shows its unfiltered state rather than rendering blank controls', () => {
    ...
    expect(screen.getByPlaceholderText('Any agent')).toBeInTheDocument();
    expect(screen.getByPlaceholderText('Any plugin')).toBeInTheDocument();
  ...
  it('filters skills by plugin, not plugins by skill', () => {
    renderFilters();

    // The 2026-09-04 session ruled out the reverse direction: the API cannot filter
    // plugin members on the organization/name pair that makes a skill unique. This
    // guards the direction that shipped.
    expect(screen.getByLabelText('Packaged in plugin')).toBeInTheDocument();
  });
  ...
  it('shows the cross-registry agent filter unfiltered rather than blank', () => {
    renderFilters();
    // Now a searchable combobox, since the agent list is unbounded, so its unfiltered
    // state is placeholder text rather than a selected option.
    expect(screen.getByPlaceholderText('Any agent')).toBeInTheDocument();
  });
