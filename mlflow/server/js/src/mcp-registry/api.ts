...
import { MCPRegistryMockApi } from './mocks/mcpRegistryMockApi';
import type {
  MCPServer,
  MCPServerVersion,
  MCPAccessEndpoint,
...
/**
 * Whether to serve this registry from the in-memory fixture instead of the tracking
 * server.
 *
 * Set at build time. The static prototype previews (GitLab Pages) have no MLflow server
 * behind them, so every call here 404s and the page renders an error; the other three
 * registries are unaffected only because they never call a server. Turning this on gives
 * MCP the same footing so the whole prototype is reviewable from a URL.
 *
 * Off by default, which keeps every normal build and every local dev session pointed at
 * the real API. To exercise the fixture locally:
 *
 *     REACT_APP_MCP_REGISTRY_MOCKS=true yarn start
 */
const useMockApi = process.env['REACT_APP_MCP_REGISTRY_MOCKS'] === 'true';

export const MCPRegistryApi = useMockApi ? MCPRegistryMockApi : MCPRegistryHttpApi;
